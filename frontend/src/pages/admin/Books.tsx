...
import { 
  ...
  Book, Edit, ArrowUpDown, Plus, Filter, RefreshCw 
} from 'lucide-react';
        ...
        <div className="bg-[#1a2032] rounded-xl shadow-sm overflow-hidden">
              ...
              <table className="w-full table-auto">
                <thead>
                  <tr className="border-b border-[#2a2f42] bg-[#1e263a]">
                    ...
                    <th className="text-left py-3 px-4 font-medium text-sm text-gray-300">Authors</th>
                    <th className="text-left py-3 px-4 font-medium text-sm text-gray-300">Categories</th>
                    <th className="text-left py-3 px-4 font-medium text-sm text-gray-300">
                      <button 
                        className="flex items-center font-medium focus:outline-none"
                      ...
                      >
                        Available
                        <span className="ml-1">{getSortIcon('available_books')}</span>
                      </button>
                    </th>
                    ...
                    <tr>
                      <td colSpan={5} className="py-12 px-4 text-center text-gray-400">
                        <div className="flex flex-col items-center justify-center">
                          <Book size={40} className="text-gray-600 mb-3" strokeWidth={1} />
                    ...
                    sortedBooks.map((book) => (
                      ...
                      >
                        <td className="py-4 px-4">
                          <div className="flex items-center">
                            <div className={`w-10 h-14 ${getCoverColor(book.id)} rounded flex items-center justify-center mr-3`}>
                              {book.cover_url ? (
                                <img
                                  src={book.cover_url}
                                  alt={book.title}
                                  ...
                                  onError={(e) => {
                                    (e.currentTarget.parentNode as HTMLElement).classList.add(getCoverColor(book.id));
                        ...
                        <td className="py-4 px-4 text-gray-300">{book.authors.length > 0 ? book.authors.join(', ') : 'Unknown Author'}</td>
                        <td className="py-4 px-4 text-gray-300">
                          <div className="flex flex-wrap gap-1">
                            {book.categories.length > 0 ? (
                              book.categories.map((category, idx) => (
                                <span key={idx} className="inline-block px-2 py-0.5 bg-[#252b3d] text-xs rounded text-gray-300">
                                  {category}
                                </span>
                              ))
                            ) : (
                        ...
                        <td className="py-4 px-4 text-gray-300">
                          <span className={`${book.available_books > 0 ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <div className="p-4 border-t border-[#2a2f42] flex justify-between items-center">
            <div className="text-sm text-gray-400">
              Showing <span className="font-medium text-gray-300">{sortedBooks.length}</span> books
            </div>
